...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    let query = supabase
      .from("assessments")
      .select("*, class:classes!inner(id, name, school_id), subject:subjects(*), term:terms(*), assessment_scores(count)")
    ...
    if (error) return NextResponse.json({ error: "Failed to fetch assessments" }, { status: 400 });
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (error) return NextResponse.json({ error: "Failed to create assessment" }, { status: 400 });
